...
def run(kc: "KimConvergence") -> None:
    ...
    if len(p.key) == 1: # try to only check single keyr
        convergence_fn = lambda kc: mcr(kc.state[p.key[0]], p.percent_past_d, p.every, p.key, kc.log)
    else: # fall back and check all keys
        convergence_fn =  lambda kc: all(mcr(kc.state[k],  p.percent_past_d, p.every, k, kc.log) for k in p.key)
    ...
    if kc.step < kc.max_steps:
        ...
        raise Warning("Increase max steps to allow for system to fully equilibrate")
